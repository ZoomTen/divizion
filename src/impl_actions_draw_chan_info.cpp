...
void DivizionActionsImpl::drawChannelInfo()
{
  ...
  if (ImGui::BeginTable("SystemList", 3)) {
    ...
    for (unsigned char i = 0; i < e->song.systemLen; i++) {
      ...
      if (ImGui::TreeNode(fmt::sprintf("%d. %s##_SYSM%d", i + 1,
                                       e->getSystemName(e->song.system[i]), i)
                            .c_str())) {
        drawSysConf(this,i, i, e->song.system[i], e->song.systemFlags[i], true);
      ...
      if (ImGui::Button(_("Clone##SysDup"))) {
        if (!e->duplicateSystem(i, sysDupCloneChannels, sysDupEnd)) {
          this->guiErrorMessage=
            fmt::sprintf(_("cannot clone chip! (%s)"), e->getLastError());
        } else {
          if (e->song.autoSystem) {
            autoDetectSystem(this);
          }
      ...
      if (ImGui::BeginPopupContextItem("SysPickerC",
                                       ImGuiPopupFlags_MouseButtonLeft)) {
        DivSystem picked = systemPicker(this,false);
        if (picked != DIV_SYSTEM_NULL) {
          if (e->changeSystem(i, picked, preserveChanPos)) {
            ...
            this->guiErrorMessage=
              fmt::sprintf(_("cannot change chip! (%s)"), e->getLastError());
          }
    ...
    if (e->song.systemLen < DIV_MAX_CHIPS) {
      ...
      if (ImGui::BeginPopupContextItem("SysPickerA",
                                       ImGuiPopupFlags_MouseButtonLeft)) {
        DivSystem picked = systemPicker(this,false);
        if (picked != DIV_SYSTEM_NULL) {
          if (!e->addSystem(picked)) {
            this->guiErrorMessage=
              fmt::sprintf(_("cannot add chip! (%s)"), e->getLastError());
          } else {
          }
          if (e->song.autoSystem) {
            autoDetectSystem(this);
...
bool drawSysConf(DivizionActionsImpl*self, int chan, int sysPos, DivSystem type, DivConfig& flags, bool modifyOnChange, bool fromMenu) {
  bool altered=false;
  ...
  switch (type) {
    ...
    case DIV_SYSTEM_YM2612_CSM: {
      ...
      if (CWSliderInt(self,_("cycles##InterruptSim"),&interruptSimCycles,0,256)) {
    ...
    case DIV_SYSTEM_GBA_DMA: {
      ...
      if (CWSliderInt(self,"##DACDepth",&dacDepth,6,9)) {
    ...
    case DIV_SYSTEM_GBA_MINMOD: {
      ...
      if (CWSliderInt(self,"##Channels",&channels,1,16)) {
        if (channels<1) channels=1;
        if (channels>16) channels=16;
        altered=true;
      } 
      ImGui::Text(_("Sample rate:"));
      if (CWSliderInt(self,"##SampRate",&sampRate,256,65536)) {
    ...
    case DIV_SYSTEM_C64_PCM: {
      ...
      if (CWSliderInt(self,_("Attack"),&testAttack,0,15)) {
        if (testAttack<0) testAttack=0;
        if (testAttack>15) testAttack=15;
        altered=true;
      }
      if (CWSliderInt(self,_("Decay"),&testDecay,0,15)) {
        if (testDecay<0) testDecay=0;
        if (testDecay>15) testDecay=15;
        altered=true;
      }
      if (CWSliderInt(self,_("Sustain"),&testSustain,0,15)) {
        if (testSustain<0) testSustain=0;
        if (testSustain>15) testSustain=15;
        altered=true;
      }
      if (CWSliderInt(self,_("Release"),&testRelease,0,15)) {
      ...
      if (CWSliderInt(self,"##InitReset",&initResetTime,0,16)) {
    ...
    case DIV_SYSTEM_YM2610B_CSM: {
      ...
      if (CWSliderInt(self,_("SSG Volume"),&ssgVol,0,256)) {
        if (ssgVol<0) ssgVol=0;
        if (ssgVol>256) ssgVol=256;
        altered=true;
      } 

      if (CWSliderInt(self,_("FM/ADPCM Volume"),&fmVol,0,256)) {
        if (fmVol<0) fmVol=0;
    ...
    case DIV_SYSTEM_AY8930: {
      ...
      if (stereo) {
        ...
        if (CWSliderInt(self,_("Separation"),&sep,1,256)) {
          if (sep<1) sep=1;
          if (sep>256) sep=256;
          stereoSep=256-sep;
          altered=true;
    ...
    case DIV_SYSTEM_AMIGA: {
      ...
      if (CWSliderInt(self,"##StereoSep",&stereoSep,0,127)) {
    ...
    case DIV_SYSTEM_QSOUND: {
      ...
      if (CWSliderInt(self,"##EchoBufSize",&echoBufSize1,0,2725)) {
      ...
      ImGui::Text(_("Echo feedback:"));
      if (CWSliderInt(self,"##EchoFeedback",&echoFeedback,0,255)) {
        if (echoFeedback<0) echoFeedback=0;
    ...
    case DIV_SYSTEM_N163: {
      ...
      if (CWSliderInt(self,"##N163_InitialChannelLimit",&channels,1,8)) {
        if (channels<1) channels=1;
    ...
    case DIV_SYSTEM_ES5506: {
      ...
      ImGui::Text(_("Initial channel limit:"));
      if (CWSliderInt(self,"##OTTO_InitialChannelLimit",&channels,5,32)) {
      ...
      if (CWSliderInt(self,"##VolScaleO",&volScale,0,4095)) {
        if (volScale<0) volScale=0;
      ...
      if (amigaPitch && self->e->song.linearPitch==2) {
        if (ImGui::IsItemHovered()) {
    ...
    case DIV_SYSTEM_YM2203_CSM: {
      ...
      if (CWSliderInt(self,_("FM Volume"),&fmVol,0,256)) {
        if (fmVol<0) fmVol=0;
        if (fmVol>256) fmVol=256;
        altered=true;
      } 
    ...
    case DIV_SYSTEM_SNES: {
      ...
      ImGui::Text(_("Volume scale:"));
      if (CWSliderInt(self,_("Left##VolScaleL"),&vsL,0,127)) {
        if (vsL<0) vsL=0;
        if (vsL>127) vsL=127;
        altered=true;
      } 
      if (CWSliderInt(self,_("Right##VolScaleL"),&vsR,0,127)) {
        if (vsR<0) vsR=0;
        if (vsR>127) vsR=127;
        altered=true;
      } 
      ...
      if (CWSliderInt(self,_("Delay##EchoDelay"),&echoDelay,0,15)) {
        if (echoDelay<0) echoDelay=0;
        if (echoDelay>15) echoDelay=15;
        altered=true;
      } 

      if (CWSliderInt(self,_("Feedback##EchoFeedback"),&echoFeedback,-128,127)) {
      ...
      ImGui::Text(_("Echo volume:"));
      if (CWSliderInt(self,_("Left##EchoVolL"),&echoVolL,-128,127)) {
        if (echoVolL<-128) echoVolL=-128;
        if (echoVolL>127) echoVolL=127;
        altered=true;
      } 
      if (CWSliderInt(self,_("Right##EchoVolL"),&echoVolR,-128,127)) {
      ...
      for (int i=0; i<8; i++) {
        ...
        if (CWSliderInt(self,temp,&echoFilter[i],-128,127)) {
      ...
      if (ImGui::Button(self->snesFilterHex?_("Hex##SNESFHex"):_("Dec##SNESFHex"))) {
        self->snesFilterHex=!self->snesFilterHex;
      }
      ImGui::SameLine();
      ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x); // wavetable text input size found here
      if (ImGui::InputText("##MMLWave",&self->mmlStringSNES[sysPos])) {
        int actualData[256];
        int discardIt=0;
        memset(actualData,0,256*sizeof(int));
        decodeMMLStrW(self->mmlStringSNES[sysPos],actualData,discardIt,self->snesFilterHex?0:-128,self->snesFilterHex?255:127,self->snesFilterHex);
       
        if (self->snesFilterHex) {
          for (int i=0; i<8; i++) {
      ...
      if (!ImGui::IsItemActive()) {
        int actualData[8];
        for (int i=0; i<8; i++) {
          if (echoFilter[i]<0 && self->snesFilterHex) {
            actualData[i]=echoFilter[i]+256;
          } else {
            actualData[i]=echoFilter[i];
          }
        }
        encodeMMLStr(self->mmlStringSNES[sysPos],actualData,8,-1,-1,self->snesFilterHex);
      }
    ...
    case DIV_SYSTEM_MSM5232: {
      ...
      if (CWSliderInt(self,_("Detune"),&detune,-127,127)) {
      ...
      for (int i=0; i<4; i++) {
        ...
        if (CWSliderInt(self,temp,&groupVol[i],0,255)) {
      ...
      if (CWSliderInt(self,_("Speed"),&vibSpeed,0,256)) {
  ...
  if (altered) {
    if (chan>=0) {
     self->e->updateSysFlags(chan,restart,mustRender);
      if (self->e->song.autoSystem) {
        autoDetectSystem(self);
      }
    }
...
void autoDetectSystemIter(DivizionActionsImpl*self, std::vector<FurnaceGUISysDef>& category, bool& isMatch, std::map<DivSystem,int>& defCountMap, std::map<DivSystem,DivConfig>& defConfMap, std::map<DivSystem,int>& sysCountMap, std::map<DivSystem,DivConfig>& sysConfMap) {
  for (FurnaceGUISysDef& j: category) {
    if (!j.orig.empty()) {
      ...
      if (defCountMap.size()==sysCountMap.size()) {
        ...
        if (isMatch) {
          logV("match found!");
          self->e->song.systemName=j.name;
          break;
        }
      }
    }
    if (!j.subDefs.empty()) autoDetectSystemIter(self,j.subDefs,isMatch,defCountMap,defConfMap,sysCountMap,sysConfMap);
    if (isMatch) break;
  }
}

void autoDetectSystem(DivizionActionsImpl*self) {
  std::map<DivSystem,int> sysCountMap;
  std::map<DivSystem,DivConfig> sysConfMap;
  for (int i=0; i<self->e->song.systemLen; i++) {
    auto it=sysCountMap.find(self->e->song.system[i]);
    if (it==sysCountMap.cend()) {
      sysCountMap[self->e->song.system[i]]=1;
    } else {
      it->second++;
    }
    sysConfMap[self->e->song.system[i]]=self->e->song.systemFlags[i];
  }

  logV("sysCountMap:");
  for (std::pair<DivSystem,int> k: sysCountMap) {
    logV("%s: %d",self->e->getSystemName(k.first),k.second);
  }

  bool isMatch=false;
  std::map<DivSystem,int> defCountMap;
  std::map<DivSystem,DivConfig> defConfMap;
  for (FurnaceGUISysCategory& i: self->sysCategories) {
    autoDetectSystemIter(self,i.systems,isMatch,defCountMap,defConfMap,sysCountMap,sysConfMap);
    if (isMatch) break;
  }

  if (!isMatch) {
    bool isFirst=true;
     self->e->song.systemName="";
    for (std::pair<DivSystem,int> k: sysCountMap) {
      if (!isFirst)  self->e->song.systemName+=" + ";
      if (k.second>1) {
         self->e->song.systemName+=fmt::sprintf("%d×",k.second);
      }
       self->e->song.systemName+= self->e->getSystemName(k.first);
      isFirst=false;
    }
  }
}

DivSystem systemPicker(DivizionActionsImpl*self,bool fullWidth) {
  DivSystem ret=DIV_SYSTEM_NULL;
  DivSystem hoveredSys=DIV_SYSTEM_NULL;
  bool reissueSearch=false;
  if (self->curSysSection==NULL) {
  ...
  if (ImGui::BeginTabBar("SysCats")) {
    for (int i=0; chipCategories[i]; i++) {
      if (ImGui::BeginTabItem(_(chipCategoryNames[i]))) {
        ...
        self->curSysSection=chipCategories[i];
        ImGui::EndTabItem();
      }
    }
    ImGui::EndTabBar();
  ...
  if (reissueSearch) {
    String lowerCase=self->sysSearchQuery;
    for (char& i: lowerCase) {
      if (i>='A' && i<='Z') i+='a'-'A';
    }
    self->sysSearchResults.clear();
    for (int j=0; self->curSysSection[j]; j++) {
      String lowerCase1=self->e->getSystemName((DivSystem)self->curSysSection[j]);
      for (char& i: lowerCase1) {
        if (i>='A' && i<='Z') i+='a'-'A';
      }
      if (lowerCase1.find(lowerCase)!=String::npos) {
        self->sysSearchResults.push_back((DivSystem)self->curSysSection[j]);
      }
    }
  }
 if (ImGui::BeginTable("SysList",1,ImGuiTableFlags_ScrollY|ImGuiTableFlags_BordersOuterH,ImVec2(fullWidth ? ImGui::GetContentRegionAvail().x : 500.0f,200.0f))) {
    if (self->sysSearchQuery.empty()) {
      // display chip list
      for (int j=0; self->curSysSection[j]; j++) {
        ImGui::TableNextRow();
        ImGui::TableNextColumn();
        if (ImGui::Selectable(self->e->getSystemName((DivSystem)self->curSysSection[j]),false,0,ImVec2(500.0f,0.0f))) ret=(DivSystem)self->curSysSection[j];
        if (ImGui::IsItemHovered() && hoveredSys==DIV_SYSTEM_NULL) {
          hoveredSys=(DivSystem)self->curSysSection[j];
        }
      }
    } else {
      // display search results
      for (DivSystem i: self->sysSearchResults) {
        ImGui::TableNextRow();
        ImGui::TableNextColumn();
        if (ImGui::Selectable(self->e->getSystemName(i),false,0,ImVec2(500.0f,0.0f))) ret=i;
        if (ImGui::IsItemHovered() && hoveredSys==DIV_SYSTEM_NULL) {
          hoveredSys=i;
        }
      }
  ...
  if (ImGui::BeginChild("SysDesc",ImVec2(0.0f,150.0f),false,ImGuiWindowFlags_NoScrollbar|ImGuiWindowFlags_NoScrollWithMouse)) {
    if (hoveredSys!=DIV_SYSTEM_NULL) {
      const DivSysDef* sysDef=self->e->getSystemDef(hoveredSys);
      ImGui::TextWrapped("%s",sysDef->description);
      ImGui::Separator();
      drawSystemChannelInfoText(sysDef);
      drawSystemChannelInfo(sysDef);
