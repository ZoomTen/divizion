...
struct FurnacePlotArrayGetterData {
  ...
  FurnacePlotArrayGetterData(const float* values, int stride)
  {
    Values = values;
    Stride = stride;
  }
...
struct FurnacePlotIntArrayGetterData {
  ...
  FurnacePlotIntArrayGetterData(const int* values, int stride)
  {
    Values = values;
    Stride = stride;
  }
...
{
  FurnacePlotIntArrayGetterData* plot_data =
    (FurnacePlotIntArrayGetterData*)data;
  const int v =
    *(const int*)(const void*)((const unsigned char*)plot_data->Values
                               + (size_t)idx * plot_data->Stride);
  return v;
}

int PlotNoLerpEx(ImGuiPlotType plot_type, const char* label,
                 ...
                 float scale_min, float scale_max, ImVec2 frame_size)
{
  ...
  if (values_count >= values_count_min) {
    ...
    const ImU32 col_base = ImGui::GetColorU32((plot_type == ImGuiPlotType_Lines)
                                                ? ImGuiCol_PlotLines
                                                : ImGuiCol_PlotHistogram);
    const ImU32 col_hovered = ImGui::GetColorU32(
    ...
    for (int n = 0; n < res_w; n++) {
      ...
      if (plot_type == ImGuiPlotType_Lines) {
        ...
        window->DrawList->AddLine(
          pos2, pos3, idx_hovered == v1_idx ? col_hovered : col_base);
      } else if (plot_type == ImGuiPlotType_Histogram) {
                ...
                float scale_max, ImVec2 graph_size, int stride)
{
  ...
  PlotNoLerpEx(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void*)&data,
               values_count, values_offset, overlay_text, scale_min, scale_max,
               graph_size);
}

int PlotBitfieldEx(const char* label, int (*values_getter)(void* data, int idx),
                   void* data, int values_count, int values_offset,
                   const char** overlay_text, int bits, ImVec2 frame_size,
                   const bool* values_highlight, ImVec4 highlightColor,
                   ImVec4 color, std::string (*hoverFunc)(int, float, void*),
                   void* hoverFuncUser)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = ImGui::GetCurrentWindow();
  ...
  if (values_count >= values_count_min) {
    ...
    float t0 = 0.0f;
    ImVec2 tp0 =
      ImVec2(t0, 0.0f); // Point in the normalized space of our target rectangle
    ...
    for (int n = 0; n < res_w; n++) {
      ...
      IM_ASSERT(v1_idx >= 0 && v1_idx < values_count);
      const int v1 =
        values_getter(data, (v1_idx + values_offset) % values_count);
      ...
      for (int o = 0; o < bits; o++) {
        ...
        ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max, tp1);
        if (pos1.x >= pos0.x + 2.0f) pos1.x -= 1.0f;
        if (pos1.y <= pos0.y - 2.0f) pos1.y += 1.0f;
        if (v1 & (1 << o)) {
          ImU32 rCol = (idx_hovered == v1_idx ? col_hovered : col_base);
          if (values_highlight != NULL) {
            if (values_highlight[v1_idx])
              rCol = ImGui::GetColorU32(highlightColor);
          }
  ...
  if (overlay_text) {
    ...
    for (int i = 0; i < bits && overlay_text[i]; i++) {
      ...
      const char* text = overlay_text[i];
      ImGui::RenderTextClipped(
        ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - lineHeight - 1),
        ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y + lineHeight - 1), text, NULL,
        NULL, ImVec2(0.0f, (0.5 + double(bits - 1 - i)) / double(bits)));
      ImGui::RenderTextClipped(
        ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - lineHeight + 1),
        ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y + lineHeight + 1), text, NULL,
        NULL, ImVec2(0.0f, (0.5 + double(bits - 1 - i)) / double(bits)));
      ...
      ImGui::PopStyleColor();
      ImGui::RenderTextClipped(
        ImVec2(frame_bb.Min.x, frame_bb.Min.y - lineHeight),
        ImVec2(frame_bb.Max.x, frame_bb.Max.y + lineHeight), text, NULL, NULL,
        ImVec2(0.0f, (0.5 + double(bits - 1 - i)) / double(bits)));
    }
  }
...
void PlotBitfield(const char* label, const int* values, int values_count,
                  int values_offset, const char** overlay_text, int bits,
                  ImVec2 graph_size, int stride, const bool* values_highlight,
                  ImVec4 highlightColor, ImVec4 color,
                  std::string (*hoverFunc)(int, float, void*),
                  void* hoverFuncUser)
{
  FurnacePlotIntArrayGetterData data(values, stride);
  PlotBitfieldEx(label, &Plot_IntArrayGetter, (void*)&data, values_count,
                 values_offset, overlay_text, bits, graph_size,
                 values_highlight, highlightColor, color, hoverFunc,
                 hoverFuncUser);
}

int PlotCustomEx(ImGuiPlotType plot_type, const char* label,
                 float (*values_getter)(void* data, int idx), void* data,
                 int values_count, int values_display_offset,
                 const char* overlay_text, float scale_min, float scale_max,
                 ImVec2 frame_size, ImVec4 color, int highlight,
                 std::string (*hoverFunc)(int, float, void*),
                 void* hoverFuncUser, bool blockMode,
                 std::string (*guideFunc)(float), const bool* values_highlight,
                 ImVec4 highlightColor)
{
  ...
  ImU32 bgColor =
    ImGui::GetColorU32(ImVec4(color.x, color.y, color.z, color.w * 0.15));
  ...
  if (values_count >= values_count_min) {
    ...
    if (hovered && inner_bb.Contains(g.IO.MousePos)) {
      ...
      if (hoverFunc) {
        std::string hoverText =
          hoverFunc(v_idx + values_display_offset, v0, hoverFuncUser);
        if (!hoverText.empty()) {
          ImGui::SetTooltip("%s", hoverText.c_str());
        }
      } else {
    ...
    float histogram_zero_line_t =
      (scale_min * scale_max < 0.0f)
        ? (1 + (blockMode ? (scale_min - 0.5) : scale_min) * inv_scale)
    ...
    if (highlight > 0) {
      window->DrawList->AddRectFilled(
        ...
        ImVec2(ImLerp(inner_bb.Min, inner_bb.Max,
                      ImVec2((highlight >= values_count)
                               ? 1
                               : (double(highlight) / double(values_count)),
                             1))),
        bgColor);
    }
    ...
    if (blockMode) {
      window->DrawList->AddLine(
        ImLerp(inner_bb.Min, inner_bb.Max, ImVec2(0.0f, histogram_zero_line_t)),
        ImLerp(inner_bb.Min, inner_bb.Max, ImVec2(1.0f, histogram_zero_line_t)),
        col_base);
    }
      ...
      ImVec2 pos1 =
        ImLerp(inner_bb.Min, inner_bb.Max,
               (plot_type == ImGuiPlotType_Lines)
                 ? tp1
                 : ImVec2(tp1.x, blockMode ? tp0.y : histogram_zero_line_t));
      if (plot_type == ImGuiPlotType_Lines) {
        window->DrawList->AddLine(
          pos0, pos1, idx_hovered == v1_idx ? col_hovered : col_base);
      } else if (plot_type == ImGuiPlotType_Histogram) {
        ...
        if (blockMode) {
          if ((int)v0 >= (int)(scale_max + 0.5)) {
            ...
            chevron[0] = ImVec2((pos1.x + pos0.x) * 0.5 - (2.0f * chScale),
                                pos1.y + 4.0f * chScale);
            chevron[1] =
              ImVec2((pos1.x + pos0.x) * 0.5, pos1.y + 2.0f * chScale);
            chevron[2] = ImVec2((pos1.x + pos0.x) * 0.5 + (2.0f * chScale),
                                pos1.y + 4.0f * chScale);
  ...
  if (guideFunc && idx_hovered != 0) {
    ...
    window->DrawList->AddText(frame_bb.Min + ImVec2(1.0f, 1.0f), 0xff000000,
                              guide.c_str());
    window->DrawList->AddText(frame_bb.Min + ImVec2(1.0f, -1.0f), 0xff000000,
                              guide.c_str());
    window->DrawList->AddText(frame_bb.Min + ImVec2(-1.0f, 1.0f), 0xff000000,
    ...
    window->DrawList->AddText(maxPos + ImVec2(1.0f, 1.0f), 0xff000000,
                              guide1.c_str());
    window->DrawList->AddText(maxPos + ImVec2(1.0f, -1.0f), 0xff000000,
                              guide1.c_str());
    window->DrawList->AddText(maxPos + ImVec2(-1.0f, 1.0f), 0xff000000,
...
void PlotCustom(const char* label, const float* values, int values_count,
                int values_offset, const char* overlay_text, float scale_min,
                float scale_max, ImVec2 graph_size, int stride, ImVec4 color,
                int highlight, std::string (*hoverFunc)(int, float, void*),
                void* hoverFuncUser, bool blockMode,
                std::string (*guideFunc)(float), const bool* values_highlight,
                ImVec4 highlightColor)
{
  FurnacePlotArrayGetterData data(values, stride);
  PlotCustomEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data,
               values_count, values_offset, overlay_text, scale_min, scale_max,
               graph_size, color, highlight, hoverFunc, hoverFuncUser,
               blockMode, guideFunc, values_highlight, highlightColor);
}
